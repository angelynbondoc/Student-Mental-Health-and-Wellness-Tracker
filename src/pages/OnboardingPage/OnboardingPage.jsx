...
function StepTerms({ onAgree, onDisagree, loading }) {
  ...
  const TNC_SECTIONS = [
    { icon: <FileText size={14} />,    title: "1. Acceptance of Terms",    body: "By accessing and using MindSpace, you agree to be bound by these Terms and Conditions. If you do not agree, you must not use this platform. Your continued use constitutes acceptance of any updates." },
    { icon: <Shield size={14} />,      title: "2. Eligibility",             body: "MindSpace is exclusively available to currently enrolled students, faculty, and staff of New Era University. Access requires a valid institutional email (@neu.edu.ph). Your account will be deactivated upon separation from the university." },
    { icon: <Users size={14} />,       title: "3. Community Standards",     body: "You agree to engage respectfully and constructively. The following are strictly prohibited:", list: ["Harassment, bullying, or threatening behavior", "Sharing false, misleading, or harmful information", "Content promoting self-harm, suicide, or dangerous behaviors", "Discrimination based on race, gender, religion, or disability", "Spam, unauthorized advertising, or repetitive content", "Impersonating other users, faculty, or university officials"] },
    { icon: <Shield size={14} />,      title: "4. Privacy & Confidentiality", body: "Personal information shared on MindSpace is governed by the NEU Privacy Policy. Anonymous posts will not display your identity to other users; however, anonymized data may be accessible to university administrators for safety purposes." },
    { icon: <AlertTriangle size={14} />, title: "5. Mental Health Disclaimer", body: "MindSpace is a peer support platform — NOT a substitute for professional mental health care. If you or someone you know is in crisis, contact the NEU Guidance and Counseling Office or call the National Crisis Hotline at 1553." },
    { icon: <BookOpen size={14} />,    title: "6. Content Ownership",       body: "You retain ownership of content you post. By posting, you grant New Era University a non-exclusive license to display your content within the platform. The university may remove content that violates these terms without prior notice." },
    { icon: <FileText size={14} />,    title: "7. Reporting & Moderation",  body: "Users are encouraged to report violating content using the in-app reporting feature. Reports are anonymous and reviewed within 48 hours. Repeated violations may result in suspension or permanent removal." },
    { icon: <AlertTriangle size={14} />, title: "8. Account Termination",   body: "The university reserves the right to suspend or terminate any account that violates these terms or poses a risk to the community. Users may also delete their account at any time through profile settings." },
  ];

  return (
    <div className="ob-section">
      <div className="ob-section-header">
        <div className="ob-section-icon ob-section-icon--blue"><ScrollText size={20} /></div>
        <div>
        ...
        {TNC_SECTIONS.map((s, i) => (
          <div className="ob-tnc-section" key={i}>
            ...
            {s.list && (
              <ul>
                {s.list.map((item, j) => (
                  <li key={j}><Check size={11} className="ob-tnc-li-check" />{item}</li>
                ))}
              </ul>
            )}
          </div>
      ...
      {!scrolledToEnd && (
        <div className="ob-scroll-hint"><ChevronDown size={14} />Scroll down to read all terms</div>
      )}
      ...
      <label className={`ob-checkbox-row ${!scrolledToEnd ? "ob-checkbox-row--locked" : ""}`}>
        ...
        <input type="checkbox" className="ob-checkbox-hidden" checked={checked} disabled={!scrolledToEnd} onChange={e => setChecked(e.target.checked)} />
        <span className="ob-checkbox-label">I have read and understood the Terms &amp; Conditions</span>
      </label>

      <div className="ob-btn-row">
        <button className="ob-btn-danger" onClick={onDisagree} disabled={loading}><X size={15} />Disagree</button>
        <button className="ob-btn-primary" onClick={onAgree} disabled={!checked || !scrolledToEnd || loading}>
          {loading ? "Please wait…" : <>Agree &amp; Continue <ChevronRight size={15} /></>}
        </button>
      </div>
      <p className="ob-hint"><AlertTriangle size={11} />Disagreeing will return you to the login page.</p>
    </div>
  );
}
...
function StepCollege({ onBack, onSelect }) {
  ...
  return (
    <div className="ob-section">
      <div className="ob-section-header">
        <div className="ob-section-icon ob-section-icon--green"><GraduationCap size={20} /></div>
        <div>
        ...
        <div className="ob-courses-grid">
          {colleges.map(c => (
            <button
              ...
              onClick={() => onSelect(c)}
            >
              <div className="ob-course-name">{shortName(c.name)}</div>
              <div className="ob-course-code">{c.name.startsWith('School') ? 'School' : 'College'}</div>
            </button>
          ))}
        </div>
      )}
      ...
      <div className="ob-btn-row">
        <button className="ob-btn-ghost" onClick={onBack}><ChevronLeft size={15} /> Back</button>
      </div>
    </div>
  );
}
...
function StepProgram({ college, onBack, onSelect }) {
  ...
  return (
    <div className="ob-section">
      <div className="ob-section-header">
        <div className="ob-section-icon ob-section-icon--green"><BookOpen size={20} /></div>
        <div>
...
function StepCommunities({ program, onBack, onSubmit, loading, error }) {
  ...
  useEffect(() => {
    async function fetch() {
      const [{ data: communities }, { data: mappings }] = await Promise.all([
        ...
        supabase.from('program_community_map').select('community_id').eq('program_id', program.id),
      ]);
  ...
  return (
    <div className="ob-section">
      <div className="ob-section-header">
        <div className="ob-section-icon ob-section-icon--green"><Users size={20} /></div>
        <div>
          <h2 className="ob-title">Your communities</h2>
          <p className="ob-subtitle">Suggested for {program.name} — add more if you like</p>
        </div>
      </div>
      ...
      {fetching ? (
        ...
        <>
                ...
                {suggested.map(c => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => toggle(c.id)}
                    >
                      <div className="ob-community-emoji">{COMMUNITY_EMOJI[c.name] ?? "🌐"}</div>
                      <div className="ob-course-name">{c.name}</div>
                      <div className="ob-course-code ob-suggested-badge">Suggested</div>
                      {isSel && <div className="ob-course-check"><Check size={11} strokeWidth={3} /></div>}
                    </button>
                  );
                })}
              </div>
          ...
          {others.length > 0 && (
            <>
              <div className="ob-community-section-label" style={{ marginTop: 16 }}>
          ...
          {selectedIds.length > 0 && (
            <p className="ob-selected-summary">
              ...
              {selectedIds.length === 1 ? " community selected" : " communities selected"}
            </p>
          )}
        </>
      )}

      {error && <div className="ob-error-box"><AlertTriangle size={14} /> {error}</div>}

      <div className="ob-btn-row">
        <button className="ob-btn-ghost" onClick={onBack} disabled={loading}><ChevronLeft size={15} /> Back</button>
        <button
          className="ob-btn-primary"
          onClick={() => onSubmit(selectedIds)}
          disabled={selectedIds.length === 0 || loading || fetching}
        >
          {loading ? "Setting up…" : <>Finish setup <ArrowRight size={15} /></>}
        </button>
      </div>
    </div>
  );
...
function StepWelcome({ displayName, courseCount, onContinue }) {
  return (
    <div className="ob-welcome">
      ...
      <div className="ob-welcome-badge"><PartyPopper size={18} />Welcome to MindSpace</div>
      ...
      <p className="ob-welcome-body">
        ...
        {courseCount === 1 ? "community" : "communities"}. Start exploring, sharing, and connecting with your peers.
      </p>
    ...
    try {
      ...
      if (selectedCommunityIds.length > 0) {
        ...
        const { error: joinError } = await supabase
          ...
          .upsert(memberships, { onConflict: 'community_id,user_id', ignoreDuplicates: true });
    ...
    <div className="ob-page">
      ...
      <div className="ob-card">
        ...
        {step === 2 && (
          <StepCollege
            onBack={() => setStep(1)}
            onSelect={college => { setCollege(college); setStep(3); }}
          />
        )}
        ...
        {step === 3 && selectedCollege && (
          <StepProgram
            ...
            onBack={() => setStep(2)}
            onSelect={program => { setProgram(program); setStep(4); }}
          />
        )}
