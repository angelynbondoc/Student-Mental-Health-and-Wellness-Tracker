...
function Avatar({ profile, size = 88, onClick }) {
  ...
  const imgStyle = !profile?.is_preset && profile?.photo_offset 
    ? {
        position: "absolute",
  ...
  return (
    ...
    >
      ...
      {onClick && (
        <div className="pp-avatar-edit-overlay"><Camera size={18} /></div>
      )}
    </div>
  );
}
  ...
  return (
    <>
      <div className="pp-community-row">
        <div className="pp-community-avatar">{community.name?.[0]?.toUpperCase() ?? "?"}</div>
...
export default function ProfilePage() {
  const { currentUser, profiles, setProfiles, posts, setPosts, communities, setCommunities } = useContext(AppContext);
  ...
  const profile = profiles.find((p) => p.id === currentUser.id) ?? {
    id: currentUser.id, display_name: currentUser.display_name, bio: "", photo_url: null,
  };

  const myPosts       = posts.filter((p) => p.user_id === currentUser.id && !p.is_shared);
  const sharedPosts   = posts.filter((p) => p.user_id === currentUser.id && p.is_shared);
  const myCommunities = communities.filter((c) => c.member_ids?.includes(currentUser.id) ?? true);
  ...
  const handleSavePhoto = ({ photo_url, offsetX = 0, offsetY = 0, scale = 1, is_preset = false }) => {
    setProfiles((prev) => prev.map((p) =>
      p.id === currentUser.id
        ? { ...p, photo_url, is_preset, photo_offset: { x: offsetX, y: offsetY, scale } }
        : p
    ));
    setPhotoModal(false);
  };
  ...
  const handleLeave = (communityId) =>
    setCommunities((prev) => prev.map((c) =>
      c.id === communityId
        ? { ...c, member_ids: (c.member_ids ?? []).filter((id) => id !== currentUser.id) }
        : c
    ));
    ...
    <div className="pp-shell">
      ...
      <div className="pp-tabs">
        {TABS.map(({ id, label, icon: Icon }) => (
          <button key={id} className={`pp-tab${activeTab === id ? " pp-tab--active" : ""}`} onClick={() => setActiveTab(id)}>
            <Icon size={15} /> {label}
          </button>
        ))}
      </div>
      ...
      <div className="pp-content">
        {activeTab === "posts" && (
          <div className="pp-list">
            {myPosts.length === 0
              ? <div className="pp-empty"><FileText size={32} className="pp-empty-icon" /><p>You haven't posted anything yet.</p></div>
              : myPosts.map((post) => (
                  <ProfilePostCard key={post.id} post={post} isShared={false}
        ...
        {activeTab === "shared" && (
          <div className="pp-list">
            {sharedPosts.length === 0
              ? <div className="pp-empty"><Repeat2 size={32} className="pp-empty-icon" /><p>No shared posts yet.</p></div>
              : sharedPosts.map((post) => (
                  <ProfilePostCard key={post.id} post={post} isShared
                    community={communities.find((c) => c.id === post.community_id)}
                    onDelete={handleDeletePost} />
                ))}
          </div>
        ...
        {activeTab === "communities" && (
          <div className="pp-list">
            {myCommunities.length === 0
              ? <div className="pp-empty"><Users size={32} className="pp-empty-icon" /><p>You haven't joined any communities.</p></div>
